...
All configuration files for this chapter are in the `developer-concepts` directory. Make sure you change to that directory before giving any commands in this chapter.
...
Create the pod:
	...
	$ kubectl apply -f templates/pod-resources.yaml
...
The max amount of memory allocated for the WildFly container in this pod is restricted to 200MB. Let's create this Pod:
	...
	$ kubectl apply -f templates/pod-resources1.yaml
...
In `templates/pod-resources2.yaml`, change the value of `spec.containers[].resources.limits.memory` to `300Mi`. Delete the existing Pod, and create a new one:

	$ kubectl delete -f templates/pod-resources1.yaml
	pod "wildfly-pod" deleted
	$ kubectl apply -f templates/pod-resources2.yaml
	pod "wildfly-pod" created
...
Create this Pod:
	...
	$ kubectl apply -f templates/pod-guaranteed.yaml
...
Create this Pod:
	...
	$ kubectl apply -f templates/pod-guaranteed2.yaml
...
Create this Pod:
	...
	$ kubectl apply -f templates/pod-burstable.yaml
...
. Create a new ResourceQuota:
	...
	$ kubectl apply -f templates/resource-quota.yaml
...
You may have to remove a previously running Pod or Deployment before attempting to create this Pod.
	...
	$ kubectl apply -f templates/pod.yaml
	Error from server (Forbidden): error when creating "templates/pod.yaml": pods "nginx-pod" is forbidden: failed quota: quota: must specify memory
...
There is an explicity memory resource defined here. Now, try to create the pod:
	...
	$ kubectl apply -f templates/pod-memory.yaml
	pod "nginx-pod" created
...
Note, how CPU and memory resources have incremeneted values.
